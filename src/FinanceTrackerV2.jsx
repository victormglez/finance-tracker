...
function Dashboard({
  ...
  const [nextTransId, setNextTransId] = useState(100);
  ...
  const saveTrans = () => {
    ...
    setAccounts((p) =>
      p.map((a) =>
        a.id !== transForm.accountId ? a : { ...a, balance: a.balance + amt },
      ),
    );
    setTransfers((p) => [
      { id: nextTransId, ...transForm, amount: amt, type: "received" },
      ...p,
    ]);
    setNextTransId((n) => n + 1);
  ...
  const SectionHead = ({
    ...
    <div
      style={{
        display: "flex",
        alignItems: "center",
        marginBottom: openSections[k] ? 0 : 10,
      }}
    >
      <button
        onClick={() => toggle(k)}
        style={{
          ...
          alignItems: "center",
          gap: 8,
          background: openSections[k] ? color + "11" : C.card,
          border: `1px solid ${openSections[k] ? color + "55" : C.border}`,
          borderRadius: openSections[k] ? "14px 14px 0 0" : 14,
          padding: "11px 14px",
          cursor: "pointer",
        }}
      >
        <span
          style={{
            ...
            fontSize: 13,
            fontWeight: 800,
            color: openSections[k] ? color : C.text,
          }}
        >
          {label}
        </span>
        ...
        <span
          style={{
            ...
            transform: openSections[k] ? "rotate(180deg)" : "rotate(0deg)",
          }}
        >
          ▾
        </span>
  ...
  const SectionBody = ({ k, color = C.accent, children }) =>
    openSections[k] ? (
      <div
        style={{
          background: C.card,
  ...
  return (
    <div style={{ padding: "0 16px 20px", maxWidth: 480, margin: "0 auto" }}>
      ...
      <SectionBody k="chart" color={C.orange}>
        ...
        >
          {MONTHLY_CHART.map((m, i) => {
            ...
            const last = i === MONTHLY_CHART.length - 1;
            return (
              <div
                key={m.month}
                style={{
              ...
              >
                ...
                <div
                  style={{
                    width: "100%",
                    height: `${h}%`,
                    background: last ? C.accent : "#2a2a50",
      ...
      <Modal
        ...
        title="📥 Registrar Transferencia Recibida"
      >
        <div style={{ fontSize: 12, color: C.sub, marginBottom: 14 }}>
          El monto se agregará automáticamente a la cuenta que elijas.
        </div>
        <Field label="¿De quién recibiste?">
          <Input
        ...
        <Field label="Monto recibido (MXN)">
          <Input
        ...
        <Field label="Depositar en cuenta">
                ...
                >
                  ...
                  <div style={{ flex: 1, textAlign: "left" }}>
                    ...
                    <div style={{ fontSize: 10, color: C.sub }}>
                      {mxn(acc.balance)}
                      {transForm.amount &&
                        !isNaN(parseFloat(transForm.amount)) &&
                        sel && (
                          <span style={{ color: C.green }}>
                            {" "}
                            → {mxn(acc.balance + parseFloat(transForm.amount))}
                          </span>
                        )}
                    </div>
                  </div>
        ...
        <SaveBtn onClick={saveTrans} color={C.green}>
      ...
      >
        ...
        {editingCat && (
          <button
            onClick={() => {
              setCategories((p) => p.filter((c) => c.id !== editingCat.id));
              setShowAddCat(false);
              setEditingCat(null);
            }}
            style={{
    ...
    if (form.isMSI && acc?.type === "credit") {
      ...
      setPlans((prev) => [
        ...prev,
        {
          ...
          accountId: acc.id,
          startDate: form.date,
          // store installment dates for reference
          installmentDates: payDates,
        },
      ]);
      setNextPlanId((n) => n + 1);

      // 2. Create one virtual expense per installment — placed on its payment date
      //    These show up in the accordion under each month but are marked as MSI installments
      let idCursor = nextId;
      const installmentExpenses = payDates.map((pd, i) => ({
        id: idCursor++,
        description: `${form.description.trim()} (${i + 1}/${numM})`,
        amount: monthly,
        date: pd, // date = the payment due date → goes into correct month
        accountId: acc.id,
        categoryId: form.categoryId,
        paymentDate: pd, // same as date (already the payment date)
        isMSIInstallment: true,
        msiPlanId: planId,
        msiIndex: i + 1,
        msiTotal: numM,
      }));
      setNextId(idCursor);
      setExpenses((prev) => [...prev, ...installmentExpenses]);
    } else {
      ...
      setExpenses((prev) => [
        ...prev,
        { id: nextId, ...form, amount: amt, paymentDate: pd },
      ]);
      setNextId((n) => n + 1);

      // If savings category + linked goal → add to goal, deduct from Nómina
      if (isSavingsCat && form.linkedGoalId) {
        setGoals((prev) =>
          prev.map((g) =>
            g.id === form.linkedGoalId ? { ...g, current: g.current + amt } : g,
          ),
        );
        ...
        if (nominaAcc)
          setAccounts((prev) =>
            prev.map((a) =>
              a.id === nominaAcc.id ? { ...a, balance: a.balance - amt } : a,
            ),
          );
      }
    }
    ...
    <div style={{ paddingBottom: 80 }}>
      ...
      <div style={{ padding: "0 20px" }}>
        ...
        {visibleByMonth.map(([mk, allEntries]) => {
          ...
          const visiblePayments = paymentsDue.filter(
            (p) => daysUntil(p.paymentDate) <= 0,
          );
              ...
              >
                <div style={{ flex: 1, textAlign: "left" }}>
                  ...
                  >
                    ...
                    {paymentsDue.filter((p) => daysUntil(p.paymentDate) > 0)
                      .length > 0 && (
                      <span
                        style={{
                          fontSize: 9,
                      ...
                      {visiblePayments.map((p) => {
                        ...
                        return (
                          <div
                            key={p.__payKey}
                            style={{
                              background: overdue ? C.redDim : C.elevated,
                              borderRadius: 13,
                              padding: "11px 13px",
                              marginBottom: 7,
                              display: "flex",
                              alignItems: "center",
                              gap: 11,
                              border: `1px solid ${overdue ? C.red + "66" : C.orange + "55"}`,
                            }}
                          >
                            <div
                              style={{
                                ...
                                height: 38,
                                borderRadius: 10,
                                background: (acc?.color || C.accent) + "22",
                                display: "flex",
                                alignItems: "center",
                                justifyContent: "center",
                                fontSize: 20,
                            ...
                            <div style={{ flex: 1 }}>
                              ...
                              >
                                ...
                                >
                                  · {p.items.length} cargos
                                </span>
                              </div>
                              <div
                                style={{
                                  fontSize: 10,
                                  color: overdue ? C.red : C.orange,
                                  fontWeight: 700,
                                  marginTop: 2,
                                }}
                              >
                                {overdue
                                  ...
                                  : d === 0
                                    ? "🔴 Vence hoy"
                                    : `📅 Venció el ${fmtDateShort(p.paymentDate)}`}
                              </div>
                            </div>
                            <div style={{ textAlign: "right" }}>
                              <div
          ...
          return (
            ...
            >
              ...
              >
                {[
                  ...
                  pd
                    ...
                    : { label: "Tipo", val: "Débito", color: C.green },
                ].map((s, i) => (
                  <div
                    key={i}
                    style={{
              ...
              {pd && (
                <div
                  style={{
                    background: d !== null && d <= 5 ? C.redDim : C.accentDim,
                    borderRadius: 12,
                    padding: "12px 14px",
                    marginBottom: 12,
      ...
      >
        ...
        <SaveBtn
          onClick={() => {
            ...
            if (isNaN(amt) || amt <= 0) return;
            const nominaAcc = accounts.find((a) => a.type === "debit");
            // Block if Nómina doesn't have enough funds
            ...
            setAccounts((prev) =>
              prev.map((a) => {
                if (a.id === payTDCForm.accountId)
                  return { ...a, balance: Math.min(0, a.balance + amt) };
                if (nominaAcc && a.id === nominaAcc.id)
            ...
            setExpenses((prev) => [
              ...prev,
              {
                ...
                amount: amt,
                date: today(),
                accountId: nominaAcc?.id || payTDCForm.accountId,
                categoryId: null,
                paymentDate: null,
                isTDCPayment: true,
              },
...
function Subscriptions({
  ...
  const save = () => {
    ...
    const acc = accounts.find((a) => a.id === form.accountId);
    const color = acc?.color || C.accent;
    if (editingSub) {
      setSubs((prev) =>
        prev.map((s) =>
          s.id === editingSub.id
            ? { ...s, ...form, amount: parseFloat(form.amount), color }
            : s,
        ),
      );
    } else {
  ...
  const deleteSub = () => {
    setSubs((prev) => prev.filter((s) => s.id !== editingSub.id));
    setShowModal(false);
  };
...
// ─── GOALS ────────────────────────────────────────────────────────────────────
function Goals({ goals, setGoals, accounts, setAccounts }) {
      ...
      p.map((g) =>
        g.id === editing.id
          ? {
              ...
              name: editForm.name,
              target: tar,
              current: isNaN(cur) ? g.current : cur,
              icon: editForm.icon,
  ...
  const deleteGoal = () => {
    setGoals((p) => p.filter((g) => g.id !== editing.id));
    setEditing(null);
  };
  ...
  const doWithdraw = (targetAccountId) => {
    const amt = parseFloat(withdrawAmt);
    if (isNaN(amt) || amt <= 0 || amt > withdrawGoal.current) return;
    // Subtract from goal
    setGoals((p) =>
      p.map((g) =>
        g.id === withdrawGoal.id
          ? { ...g, current: Math.max(0, g.current - amt) }
          : g,
      ),
    );
    // Add to debit account balance
    setAccounts((p) =>
      p.map((a) =>
        a.id === targetAccountId ? { ...a, balance: a.balance + amt } : a,
      ),
    );
...
// ─── MSI ──────────────────────────────────────────────────────────────────────
function MSI({ plans, setPlans, accounts }) {
  const [showModal, setShowModal] = useState(false);
  ...
  const save = () => {
    ...
    setPlans((p) => [
      ...p,
      {
        ...
        totalM: parseInt(form.months),
        paidM: 0,
        accountId: form.accountId,
      },
    ]);
    setNextId((n) => n + 1);
    setForm({ desc: "", total: "", monthly: "", months: "12", accountId: 2 });
    setShowModal(false);
  };
  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop: "auto", padding: "12px 14px" }}>
        <div
          style={{ fontSize: 11, color: C.muted, textTransform: "capitalize" }}
        >
          {new Date().toLocaleDateString("es-MX", {
            weekday: "long",
            day: "numeric",
...
export default function App() {
  ...
  const [expenses, setExpenses] = useState(
    INIT_EXPENSES.map((e) => {
      ...
      const pd =
        acc?.type === "credit" && acc.cutDay && acc.payDay
          ? calcPaymentDate(e.date, acc.cutDay, acc.payDay)
          : null;
      return { ...e, paymentDate: pd };
    }),
  );
  const [categories, setCategories] = useState(
    INIT_CATEGORIES.map((c) => ({ ...c })),
  );
  ...
  useEffect(() => {
    ...
    const request = async () => {
      ...
      accounts
        .filter((a) => a.type === "credit" && a.payDay)
        .forEach((acc) => {
  ...
  const addAccount = (data) => {
    const id = accounts.reduce((m, a) => Math.max(m, a.id), 0) + 1;
    setAccounts((p) => [...p, { id, ...data }]);
  };
  const updateAccount = (u) => {
  ...
  const screen = (
    <>
      ...
      {tab === "msi" && (
        <MSI plans={plans} setPlans={setPlans} accounts={accounts} />
      )}
      {tab === "goals" && (
        <Goals
          goals={goals}
