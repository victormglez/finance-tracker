...
// ─── DB HELPERS ───────────────────────────────────────────────────────────────
const db = {
  get:    (table, q={})  => sb.from(table).select(q.select||"*").order(q.order||"created_at", { ascending: q.asc ?? true }),
...
const C = {
  ...
  accent: "#7C6FFF", accentDim: "rgba(124,111,255,0.12)",
  green: "#00D08A", greenDim: "rgba(0,208,138,0.12)",
  red: "#FF4B6B", redDim: "rgba(255,75,107,0.12)",
  orange: "#FF9F43", orangeDim: "rgba(255,159,67,0.12)",
  blue: "#54A0FF", gold: "#FFD700",
};
...
function fmtDate(str) {
  return parseDate(str).toLocaleDateString("es-MX", { day: "numeric", month: "short", year: "numeric" });
}
function fmtDateShort(str) {
  return parseDate(str).toLocaleDateString("es-MX", { day: "numeric", month: "short" });
}
...
// ─── INITIAL DATA ─────────────────────────────────────────────────────────────
const INIT_ACCOUNTS = [
  { id:1, name:"Nómina",  type:"debit",  balance:187.00,    limit:null,  color:"#4CAF50", cutDay:null, payDay:null },
  { id:2, name:"BBVA",   type:"credit", balance:-20555.99, limit:87200, color:"#1976D2", cutDay:12,   payDay:7   },
  { id:3, name:"NU",     type:"credit", balance:-1130.00,  limit:13800, color:"#820AD1", cutDay:23,   payDay:16  },
  { id:4, name:"LikeU",  type:"credit", balance:-996.48,   limit:3000,  color:"#FF6B35", cutDay:18,   payDay:12  },
  { id:5, name:"AMEX",   type:"credit", balance:-3341.18,  limit:30000, color:"#00796B", cutDay:20,   payDay:15  },
];

const INIT_CATEGORIES = [
  { id:1, name:"Gastos Fijos",     icon:"🏠", color:"#FF5252", budget:2500  },
  { id:2, name:"Gastos Variables", icon:"🛒", color:"#FF9800", budget:18000 },
...
const INIT_SUBS = [
  { id:1, name:"Telcel",    amount:599.00,  frequency:"monthly", categoryId:1, accountId:2, chargeDay:5,  color:"#1976D2", active:true },
  { id:2, name:"Spotify",   amount:120.00,  frequency:"monthly", categoryId:1, accountId:3, chargeDay:15, color:"#1DB954", active:true },
  { id:3, name:"iCloud",    amount:49.00,   frequency:"monthly", categoryId:1, accountId:3, chargeDay:3,  color:"#555555", active:true },
  { id:4, name:"Google One",amount:59.00,   frequency:"monthly", categoryId:1, accountId:2, chargeDay:22, color:"#4285F4", active:true },
  { id:5, name:"Crossfit",  amount:1490.00, frequency:"monthly", categoryId:1, accountId:2, chargeDay:1,  color:"#FF6B35", active:true },
];

const INIT_EXPENSES = [
  { id:1,  description:"VISA Canadá",            amount:88.09,   date:"2026-06-30", accountId:3, categoryId:2 },
  { id:2,  description:"Claude AI",              amount:344.26,  date:"2026-06-25", accountId:4, categoryId:1 },
  { id:3,  description:"Desayuno Día de Madres", amount:849.20,  date:"2026-06-18", accountId:2, categoryId:2 },
  { id:4,  description:"Costco Galletas",        amount:183.11,  date:"2026-06-18", accountId:2, categoryId:2 },
  { id:5,  description:"Carnicería",             amount:446.66,  date:"2026-06-18", accountId:2, categoryId:2 },
  { id:6,  description:"Tacos Culichi",          amount:444.40,  date:"2026-06-18", accountId:2, categoryId:3 },
  { id:7,  description:"Cinturón y Cinta",       amount:913.17,  date:"2026-06-18", accountId:3, categoryId:2 },
  { id:8,  description:"Spartan SLP",            amount:2258.19, date:"2026-06-18", accountId:5, categoryId:2 },
  { id:9,  description:"Gasolina",               amount:1200.00, date:"2026-06-10", accountId:2, categoryId:2 },
  { id:10, description:"Netflix",                amount:219.00,  date:"2026-06-05", accountId:2, categoryId:1 },
];

const INIT_GOALS = [
  { id:1, name:"Cajita Turbo",       target:25000,  current:25000,    icon:"🚀", color:"#00D08A" },
  { id:2, name:"Anillo",             target:50000,  current:49437.42, icon:"💍", color:"#FFD700" },
  { id:3, name:"Boda",               target:200000, current:29260.78, icon:"💒", color:"#FF6B9D" },
  { id:4, name:"Fondo de Emergencia",target:50000,  current:9320.54,  icon:"🛡️", color:"#FF5252" },
];

const INIT_MSI = [
  { id:1, desc:"Monitor Samsung",     total:942.00,  monthly:78.50,  totalM:12, paidM:12, accountId:2 },
  { id:2, desc:"RTX 5070",            total:1287.00, monthly:107.25, totalM:18, paidM:12, accountId:2 },
  { id:3, desc:"Seguro Coche",        total:2258.00, monthly:376.33, totalM:6,  paidM:6,  accountId:2 },
  { id:4, desc:"Wallet North Face",   total:203.00,  monthly:67.67,  totalM:3,  paidM:3,  accountId:2 },
  { id:5, desc:"Plan de Retiro 26-27",total:1989.64, monthly:165.80, totalM:12, paidM:3,  accountId:2 },
];

const MONTHLY_CHART = [
  {month:"Ene",total:8200},{month:"Feb",total:9600},{month:"Mar",total:8800},
  {month:"Abr",total:14000},{month:"May",total:28000},{month:"Jun",total:6700},
];
...
function Tag({children, color}) {
  return <span style={{background:color+"22",color,fontSize:10,fontWeight:700,padding:"2px 6px",borderRadius:4}}>{children}</span>;
}
function Badge({children, color=C.orange}) {
  return <span style={{background:color+"22",color,fontSize:9,fontWeight:800,padding:"2px 7px",borderRadius:99,letterSpacing:.3,whiteSpace:"nowrap"}}>{children}</span>;
}
    ...
    <div style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,.8)",display:"flex",alignItems:"flex-end",justifyContent:"center",backdropFilter:"blur(4px)"}}
      ...
      <div style={{background:C.elevated,borderRadius:"22px 22px 0 0",width:"100%",maxWidth:480,maxHeight:"92vh",padding:22,paddingTop:10,paddingBottom:"calc(22px + env(safe-area-inset-bottom, 0px))",overflowY:"auto",border:`1px solid ${C.borderLight}`,borderBottom:"none",animation:"slideUp .28s cubic-bezier(.4,0,.2,1)"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          ...
          <button onClick={onClose} style={{background:"none",border:"none",color:C.sub,fontSize:24,cursor:"pointer",lineHeight:1,padding:2}}>×</button>
        </div>
        {children}
      </div>
    </div>
  ...
  return (
    <div style={{marginBottom:14}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:6}}>
        <span style={{fontSize:11,fontWeight:700,color:C.sub,textTransform:"uppercase",letterSpacing:.8}}>{label}</span>
...
function Input({value,onChange,placeholder,type="text",style={}}) {
  return (
    <input type={type} value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder}
      style={{width:"100%",background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:"10px 12px",color:C.text,fontSize:14,outline:"none",boxSizing:"border-box",fontFamily:"inherit",...style}}/>
  );
}

function Stepper({value,onChange,min=1,max=31,label}) {
  return (
    <div style={{display:"flex",alignItems:"center",gap:10,background:C.card,borderRadius:10,border:`1px solid ${C.border}`,padding:"6px 12px"}}>
      <button onClick={()=>onChange(Math.max(min,value-1))} style={{background:C.elevated,border:"none",borderRadius:6,width:28,height:28,color:C.text,fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>−</button>
      <span style={{flex:1,textAlign:"center",fontSize:20,fontWeight:800,color:C.text}}>{value}</span>
      <button onClick={()=>onChange(Math.min(max,value+1))} style={{background:C.elevated,border:"none",borderRadius:6,width:28,height:28,color:C.text,fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
    </div>
  );
}
...
function ChipSelect({options,value,onChange,getColor,getLabel}) {
  return (
    <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
      {options.map(opt=>{
        const key = opt.id??opt; const active = value===key;
        const col = getColor?.(opt)||C.accent;
        return (
          <button key={key} onClick={()=>onChange(key)} style={{padding:"6px 11px",borderRadius:99,fontSize:12,fontWeight:700,cursor:"pointer",border:"none",background:active?col:C.card,color:active?"#fff":C.sub,outline:active?`1px solid ${col}`:`1px solid ${C.border}`,transition:"all .15s"}}>
...
function SaveBtn({onClick,children,color=C.accent}) {
  return (
    <button onClick={onClick} style={{width:"100%",background:color,border:"none",borderRadius:12,padding:"13px 0",color:"#fff",fontSize:14,fontWeight:800,cursor:"pointer",marginTop:8,letterSpacing:.3}}
...
function AccountCard({acc, onClick}) {
  ...
  return (
    <div onClick={onClick} style={{background:C.card,borderRadius:18,padding:14,border:`1px solid ${C.border}`,borderLeft:`3px solid ${acc.color}`,cursor:"pointer",transition:"transform .1s"}}
      ...
      {acc.cutDay&&<div style={{marginTop:6,display:"flex",gap:4,flexWrap:"wrap"}}>
        {dCut!==null&&<Badge color={C.orange}>Corte: {dCut===0?"Hoy":`${dCut}d`}</Badge>}
        {dPay!==null&&<Badge color={dPay<=3?C.red:C.blue}>Pago: {dPay===0?"Hoy":`${dPay}d`}</Badge>}
      </div>}
    </div>
  );
}
...
function AccountModal({open, onClose, onSave, onDelete, editAccount=null}) {
  ...
  const COLORS = ["#4CAF50","#1976D2","#820AD1","#FF6B35","#00796B","#FF5252","#FF9F43","#54A0FF","#E91E63","#7C6FFF"];

  const [step, setStep] = useState(1);
  const [form, setForm] = useState({name:"",type:"debit",limit:"",color:"#4CAF50",cutDay:12,payDay:7,balance:""});
  ...
  useEffect(() => {
    if (open) {
      if (isEdit) {
        ...
        setForm({name:"",type:"debit",limit:"",color:"#4CAF50",cutDay:12,payDay:7,balance:"0"});
        setStep(1);
      }
    }
  }, [open]);
    ...
    <Modal open={open} onClose={handleClose} title={titleMap[step]||"Cuenta"}>
      {step===1&&<>
        <Field label="Nombre de la cuenta">
          <Input value={form.name} onChange={v=>setForm(f=>({...f,name:v}))} placeholder="Ej. BBVA, Nu, AMEX..."/>
        </Field>

        {/* Only allow type change when creating */}
        {!isEdit && <Field label="Tipo de cuenta">
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {[
              {type:"debit", icon:"🏦", label:"Débito",  desc:"Saldo disponible"},
              {type:"credit",icon:"💳", label:"Crédito", desc:"Línea de crédito"},
            ].map(opt=>{
              const active=form.type===opt.type;
              return (
                <button key={opt.type} onClick={()=>setForm(f=>({...f,type:opt.type}))}
                  style={{background:active?C.accentDim:C.card,border:active?`1.5px solid ${C.accent}`:`1px solid ${C.border}`,borderRadius:14,padding:"14px 10px",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:6}}>
                  <span style={{fontSize:28}}>{opt.icon}</span>
                  <span style={{fontSize:13,fontWeight:800,color:active?C.accent:C.text}}>{opt.label}</span>
                  <span style={{fontSize:10,color:C.sub}}>{opt.desc}</span>
                </button>
              );
            })}
          </div>
        ...
        {isEdit && <div style={{display:"flex",alignItems:"center",gap:8,background:C.card,borderRadius:10,padding:"10px 14px",marginBottom:14,border:`1px solid ${C.border}`}}>
          <span style={{fontSize:20}}>{form.type==="debit"?"🏦":"💳"}</span>
          <span style={{fontSize:13,fontWeight:700,color:C.text}}>{form.type==="debit"?"Cuenta Débito":"Tarjeta de Crédito"}</span>
          <span style={{marginLeft:"auto",fontSize:10,color:C.muted}}>no editable</span>
        </div>}

        <Field label={form.type==="debit"?"Saldo actual (MXN)":"Saldo / Deuda actual (MXN)"}>
          <Input value={form.balance} onChange={v=>setForm(f=>({...f,balance:v}))} placeholder={form.type==="debit"?"1500.00":"-5200.00"} type="number"/>
          {form.type==="credit"&&<p style={{fontSize:11,color:C.sub,margin:"5px 0 0"}}>Usa número negativo para deuda (ej. -5200)</p>}
        </Field>

        <div style={{display:"flex",gap:8}}>
          {isEdit&&onDelete&&<button onClick={()=>{if(window.confirm("¿Eliminar esta cuenta?"))onDelete();}} style={{flex:1,background:C.redDim,border:`1px solid ${C.red}44`,borderRadius:12,padding:"12px 0",color:C.red,fontSize:13,fontWeight:700,cursor:"pointer"}}>🗑</button>}
          <button onClick={()=>setStep(isCredit?2:3)} style={{flex:3,background:C.accent,border:"none",borderRadius:12,padding:"12px 0",color:"#fff",fontSize:13,fontWeight:800,cursor:"pointer"}}>Siguiente →</button>
        </div>
      </>}

      {step===2&&isCredit&&<>
        <Field label="Límite de crédito (MXN)">
          <Input value={form.limit} onChange={v=>setForm(f=>({...f,limit:v}))} placeholder="Ej. 30000" type="number"/>
        </Field>
        <Field label="Día de corte" hint="Fin del ciclo de facturación">
          <Stepper value={form.cutDay} onChange={v=>setForm(f=>({...f,cutDay:v}))}/>
          <p style={{fontSize:11,color:C.sub,margin:"6px 0 0"}}>Gastos antes del día <b style={{color:C.orange}}>{form.cutDay}</b> entran al ciclo actual</p>
        </Field>
        <Field label="Día límite de pago" hint="Fecha máxima para pagar sin intereses">
          <Stepper value={form.payDay} onChange={v=>setForm(f=>({...f,payDay:v}))}/>
          <p style={{fontSize:11,color:C.sub,margin:"6px 0 0"}}>Tu pago vence el <b style={{color:C.red}}>día {form.payDay}</b> de cada mes</p>
        </Field>
        <div style={{display:"flex",gap:8}}>
          <button onClick={()=>setStep(1)} style={{flex:1,background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 0",color:C.sub,fontSize:13,fontWeight:700,cursor:"pointer"}}>← Atrás</button>
          <button onClick={()=>setStep(3)} style={{flex:2,background:C.accent,border:"none",borderRadius:12,padding:"12px 0",color:"#fff",fontSize:13,fontWeight:800,cursor:"pointer"}}>Siguiente →</button>
        </div>
      </>}
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:10,justifyContent:"center",padding:"8px 0"}}>
            {COLORS.map(col=>(
              <button key={col} onClick={()=>setForm(f=>({...f,color:col}))}
                style={{width:40,height:40,borderRadius:20,background:col,border:"none",cursor:"pointer",outline:form.color===col?"3px solid #fff":"2px solid transparent",outlineOffset:2,transition:"all .15s"}}/>
            ))}
          </div>
        </Field>
        {/* Live preview */}
        <div style={{background:C.card,borderRadius:14,padding:14,border:`1px solid ${C.border}`,borderLeft:`3px solid ${form.color}`,marginBottom:16}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
            <span style={{fontSize:20}}>{form.type==="debit"?"🏦":"💳"}</span>
            <span style={{fontSize:15,fontWeight:800,color:C.text}}>{form.name||"Mi Cuenta"}</span>
            <span style={{marginLeft:"auto",fontSize:9,background:form.color+"22",color:form.color,padding:"2px 6px",borderRadius:4,fontWeight:700}}>{form.type==="debit"?"Débito":"Crédito"}</span>
          </div>
          {form.balance!==""&&<div style={{fontSize:16,fontWeight:800,color:parseFloat(form.balance)>=0?C.text:C.red}}>{mxn(parseFloat(form.balance)||0)}</div>}
          {isCredit&&<div style={{fontSize:11,color:C.sub,marginTop:4}}>Corte: día {form.cutDay} · Pago: día {form.payDay}</div>}
        </div>
        <div style={{display:"flex",gap:8}}>
          <button onClick={()=>setStep(isCredit?2:1)} style={{flex:1,background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 0",color:C.sub,fontSize:13,fontWeight:700,cursor:"pointer"}}>← Atrás</button>
          <button onClick={save} style={{flex:2,background:form.color,border:"none",borderRadius:12,padding:"12px 0",color:"#fff",fontSize:13,fontWeight:800,cursor:"pointer"}}>
...
function UpcomingPayments({expenses, accounts, subs}) {
  ...
  const creditAccounts = accounts.filter(a=>a.type==="credit"&&a.cutDay&&a.payDay);

  const paymentGroups = creditAccounts.map(acc => {
  ...
  const subCharges = subs.filter(s=>s.active).map(sub=>{
  ...
  return (
    <div style={{marginBottom:24}}>
      <div style={{fontSize:13,fontWeight:800,color:C.sub,textTransform:"uppercase",letterSpacing:1,marginBottom:12}}>📅 Próximos Pagos</div>

      {/* Credit card payments */}
      {paymentGroups.map(({acc, byPayDate})=>(
        Object.entries(byPayDate).sort(([a],[b])=>a.localeCompare(b)).map(([pd,data])=>{
          const d = daysUntil(pd);
          const urgent = d<=5;
          return (
            <div key={acc.id+pd} style={{background:C.card,borderRadius:14,padding:"12px 14px",marginBottom:8,border:`1px solid ${urgent?C.red+"55":C.border}`,display:"flex",alignItems:"center",gap:12}}>
              <div style={{width:40,height:40,borderRadius:12,background:acc.color+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>💳</div>
              <div style={{flex:1}}>
                <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}>
                  <span style={{fontSize:13,fontWeight:800,color:C.text}}>{acc.name}</span>
                  <Tag color={acc.color}>{data.count} cargo{data.count>1?"s":""}</Tag>
                </div>
                <div style={{fontSize:11,color:C.sub}}>Vence: {fmtDate(pd)}</div>
              </div>
              <div style={{textAlign:"right"}}>
                <div style={{fontSize:16,fontWeight:900,color:urgent?C.red:C.text}}>{mxn(data.amount)}</div>
                <div style={{fontSize:10,fontWeight:700,color:urgent?C.red:C.muted}}>{d===0?"🔴 Hoy":d<0?`⚠️ Vencido`:`${d}d`}</div>
              </div>
            </div>
          );
        })
      ...
      {subCharges.slice(0,3).map(({sub,acc,nextCharge,daysAway})=>(
        <div key={sub.id} style={{background:C.card,borderRadius:14,padding:"12px 14px",marginBottom:8,border:`1px solid ${daysAway<=3?C.orange+"55":C.border}`,display:"flex",alignItems:"center",gap:12}}>
          <div style={{width:40,height:40,borderRadius:12,background:C.accentDim,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>🔄</div>
          <div style={{flex:1}}>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}>
              <span style={{fontSize:13,fontWeight:800,color:C.text}}>{sub.name}</span>
              {daysAway<=3&&<Badge color={C.orange}>Pronto</Badge>}
            </div>
            <div style={{fontSize:11,color:C.sub}}>Cargo: {fmtDate(nextCharge)} {acc&&`· ${acc.name}`}</div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:15,fontWeight:800,color:C.text}}>{mxn(sub.amount)}</div>
            <div style={{fontSize:10,color:C.muted}}>{daysAway===0?"Hoy":`${daysAway}d`}</div>
          </div>
        </div>
      ))}
    </div>
...
function Dashboard({expenses, accounts, setAccounts, categories, setCategories, transfers, setTransfers, onAddAccount, onUpdateAccount, onDeleteAccount, session, reloadAll}) {
  ...
  const emptyTransForm={type:"received",amount:"",accountId:accounts[0]?.id||"",counterparty:"",date:today(),notes:""};
  ...
  const CAT_ICONS=["🏠","🛒","💕","💰","🚗","✈️","🍔","🏋️","💊","🎮","👔","📚","🐾","🎁","⚡","📱","🏥","🎵"];
  const CAT_COLORS=["#FF5252","#FF9800","#E91E63","#4CAF50","#1976D2","#820AD1","#FF6B35","#00796B","#FF9F43","#54A0FF","#7C6FFF","#FFD700","#00D08A","#F06292","#26C6DA","#AB47BC"];
  const emptyCatForm={name:"",budget:"",icon:"🛒",color:"#FF9800"};
  const [catForm,setCatForm]=useState(emptyCatForm);

  // ── Accordion open states ──
  const [openSections,setOpenSections]=useState({accounts:true,chart:true,transfers:false,cats:false});
  const toggle=(k)=>setOpenSections(s=>({...s,[k]:!s[k]}));

  // ── Handlers ──
  const saveTrans=async()=>{
    if(!transForm.amount||!transForm.counterparty.trim())return;
    const amt=parseFloat(transForm.amount);if(isNaN(amt)||amt<=0)return;
    const isRec=transForm.type==="received";
    await sb.from("transfers").insert({ user_id:session?.user?.id, type:transForm.type, amount:amt, account_id:transForm.accountId||null, counterparty:transForm.counterparty.trim(), date:transForm.date, notes:transForm.notes||null });
    const acc=accounts.find(a=>a.id===transForm.accountId);
    if(acc) await sb.from("accounts").update({balance:acc.balance+(isRec?amt:-amt)}).eq("id",acc.id);
    setTransForm(emptyTransForm);setShowAddTrans(false);
    if(reloadAll)await reloadAll();
  };
  const saveCat=async()=>{
    if(!catForm.name.trim())return;
    const budget=parseFloat(catForm.budget)||0;
    if(editingCat){await sb.from("categories").update({name:catForm.name.trim(),budget,icon:catForm.icon,color:catForm.color}).eq("id",editingCat.id);}
    else{await sb.from("categories").insert({user_id:session?.user?.id,name:catForm.name.trim(),budget,icon:catForm.icon,color:catForm.color});}
    setEditingCat(null);setShowAddCat(false);
    if(reloadAll)await reloadAll();
  };
  ...
  return (
    <div style={{padding:"0 16px 20px",maxWidth:480,margin:"0 auto"}}>
      {/* Header */}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:18,paddingTop:20}}>
        <div>
          <div style={{fontSize:26,fontWeight:900,color:C.text,letterSpacing:-.5}}>Finance Tracker</div>
          <div style={{fontSize:13,color:C.sub,marginTop:2}}>{new Date().toLocaleDateString("es-MX",{weekday:"long",day:"numeric",month:"long"})}</div>
        </div>
        <div style={{background:C.greenDim,borderRadius:14,padding:10,fontSize:22}}>📊</div>
      </div>
      ...
      <SectionBody k="cats" color="#FF9800">
        {catBreakdown.map(cat=>{
          ...
          const over=pct>100;
          return(
            <div key={cat.id} style={{padding:"9px 0",borderBottom:`1px solid ${C.border}`}}>
              <div style={{display:"flex",alignItems:"center",gap:9,marginBottom:6}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:10,color:over?C.red:C.sub,fontWeight:600}}>
                    {mxn(cat.spent)} <span style={{color:C.muted}}>/ {mxn(cat.budget)} presupuesto</span>
                    {over&&<span style={{color:C.red,fontWeight:800}}> · +{mxn(cat.spent-cat.budget)}</span>}
                  </div>
                </div>
                <span style={{fontSize:11,fontWeight:800,color:over?C.red:C.green,flexShrink:0}}>{pct.toFixed(0)}%</span>
                <button onClick={()=>{setEditingCat(cat);setCatForm({name:cat.name,budget:String(cat.budget),icon:cat.icon,color:cat.color});setShowAddCat(true);}} style={{background:C.elevated,border:`1px solid ${C.border}`,borderRadius:7,padding:"4px 7px",color:C.sub,fontSize:12,cursor:"pointer",flexShrink:0}}>✏️</button>
              </div>
              <ProgressBar pct={pct} color={over?C.red:cat.color} h={5}/>
            </div>
          );
        })}
        {categories.length===0&&<div style={{textAlign:"center",padding:"10px 0",color:C.muted,fontSize:12}}>Sin categorías</div>}
      </SectionBody>

      <div style={{height:20}}/>

      {/* ──── MODALS ──── */}
      <AccountModal open={showAddAcc} onClose={()=>setShowAddAcc(false)} onSave={(data)=>{onAddAccount(data);setShowAddAcc(false);}}/>
      <AccountModal open={!!editingAcc} onClose={()=>setEditingAcc(null)} editAccount={editingAcc} onSave={(data)=>{onUpdateAccount({...editingAcc,...data});setEditingAcc(null);}} onDelete={()=>{onDeleteAccount(editingAcc.id);setEditingAcc(null);}}/>

      <Modal open={showAddTrans} onClose={()=>setShowAddTrans(false)} title="Nueva Transferencia">
        {/* Type toggle */}
        <Field label="Tipo">
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:4}}>
            {[["received","📥","Recibí",C.green],["sent","📤","Envié",C.red]].map(([type,icon,label,col])=>{
              const active=transForm.type===type;
              return <button key={type} onClick={()=>setTransForm(f=>({...f,type}))} style={{background:active?col+"22":C.card,border:`1.5px solid ${active?col:C.border}`,borderRadius:13,padding:"12px 8px",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                <span style={{fontSize:22}}>{icon}</span>
                <span style={{fontSize:12,fontWeight:800,color:active?col:C.text}}>{label}</span>
              </button>;
            })}
          </div>
        </Field>
        ...
        <Field label="Monto (MXN)"><Input value={transForm.amount} onChange={v=>setTransForm(f=>({...f,amount:v}))} placeholder="$0.00" type="number"/></Field>
        <Field label={transForm.type==="received"?"Depositar en":"Retirar de"}>
          <div style={{display:"flex",flexDirection:"column",gap:7}}>
            {accounts.map(acc=>{const sel=transForm.accountId===acc.id; const isRec=transForm.type==="received"; const newBal=transForm.amount&&!isNaN(parseFloat(transForm.amount))&&sel ? acc.balance+(isRec?parseFloat(transForm.amount):-parseFloat(transForm.amount)) : null; return(
              <button key={acc.id} onClick={()=>setTransForm(f=>({...f,accountId:acc.id}))} style={{display:"flex",alignItems:"center",gap:10,background:sel?acc.color+"22":C.card,border:`1.5px solid ${sel?acc.color:C.border}`,borderRadius:11,padding:"9px 12px",cursor:"pointer"}}>
                <span style={{fontSize:18}}>{acc.type==="debit"?"🏦":"💳"}</span>
                <div style={{flex:1,textAlign:"left"}}>
                  <div style={{fontSize:12,fontWeight:800,color:C.text}}>{acc.name}</div>
                  <div style={{fontSize:10,color:C.sub}}>{mxn(acc.balance)}{newBal!==null&&<span style={{color:isRec?C.green:C.red}}> → {mxn(newBal)}</span>}</div>
                </div>
        ...
        <Field label="Fecha"><Input value={transForm.date} onChange={v=>setTransForm(f=>({...f,date:v}))} type="date"/></Field>
        <Field label="Notas (opcional)"><Input value={transForm.notes} onChange={v=>setTransForm(f=>({...f,notes:v}))} placeholder="Ej. Pago de renta..."/></Field>
        <SaveBtn onClick={saveTrans} color={transForm.type==="received"?C.green:C.red}>{transForm.type==="received"?"Registrar Recibida":"Registrar Enviada"}</SaveBtn>
      </Modal>

      <Modal open={showAddCat} onClose={()=>{setShowAddCat(false);setEditingCat(null);}} title={editingCat?"Editar Categoría":"Nueva Categoría"}>
        <Field label="Nombre"><Input value={catForm.name} onChange={v=>setCatForm(f=>({...f,name:v}))} placeholder="Ej. Transporte..."/></Field>
        <Field label="Presupuesto mensual (MXN)"><Input value={catForm.budget} onChange={v=>setCatForm(f=>({...f,budget:v}))} placeholder="0.00" type="number"/></Field>
        <Field label="Ícono"><IconPicker value={catForm.icon} onChange={v=>setCatForm(f=>({...f,icon:v}))} color={catForm.color} icons={CAT_ICONS}/></Field>
        <Field label="Color"><ColorPicker value={catForm.color} onChange={v=>setCatForm(f=>({...f,color:v}))} cols={CAT_COLORS}/></Field>
        <div style={{background:C.elevated,borderRadius:11,padding:"9px 12px",marginBottom:12,border:`1px solid ${C.border}`,borderLeft:`3px solid ${catForm.color}`,display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:18}}>{catForm.icon}</span><div><div style={{fontSize:13,fontWeight:800,color:C.text}}>{catForm.name||"Categoría"}</div><div style={{fontSize:11,color:catForm.color,fontWeight:700}}>{catForm.budget?mxn(parseFloat(catForm.budget)||0):mxn(0)}/mes</div></div></div>
        {editingCat&&<button onClick={async()=>{await sb.from("categories").delete().eq("id",editingCat.id);setShowAddCat(false);setEditingCat(null);if(reloadAll)await reloadAll();}} style={{width:"100%",background:C.redDim,border:`1px solid ${C.red}44`,borderRadius:12,padding:"10px 0",color:C.red,fontSize:13,fontWeight:700,cursor:"pointer",marginBottom:8}}>🗑 Eliminar</button>}
        <SaveBtn onClick={saveCat} color={catForm.color}>{editingCat?"Guardar Cambios":"Crear Categoría"}</SaveBtn>
      </Modal>
    </div>
  );
}
...
function ExpenseRow({exp, accounts, categories, onClick}) {
  ...
  const iconBg = isTDC ? C.green+"22" : isMSI ? C.orange+"22" : (cat?.color||C.accent)+"22";
  const amtColor = isTDC ? C.green : isMSI ? C.orange : C.red;
  const borderColor = isTDC ? C.green+"44" : isMSI ? C.orange+"44" : urgent ? C.orange+"55" : C.border;
  const rowBg = isTDC ? C.greenDim : isMSI ? C.elevated : C.card;

  return (
    <div onClick={onClick} style={{background:rowBg,borderRadius:13,padding:"11px 13px",marginBottom:7,display:"flex",alignItems:"center",gap:11,border:`1px solid ${borderColor}`,cursor:"pointer",transition:"opacity .15s"}}
      onMouseEnter={e=>e.currentTarget.style.opacity=".85"} onMouseLeave={e=>e.currentTarget.style.opacity="1"}>
      <div style={{width:38,height:38,borderRadius:10,background:iconBg,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>{iconEl}</div>
      <div style={{flex:1,minWidth:0}}>
        <div style={{fontSize:14,fontWeight:700,color:C.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{exp.description}</div>
        <div style={{display:"flex",gap:5,alignItems:"center",marginTop:3,flexWrap:"wrap"}}>
          {isTDC && <Badge color={C.green}>Pago TDC</Badge>}
          {isMSI && <Badge color={C.orange}>MSI {exp.msiIndex}/{exp.msiTotal}</Badge>}
          {!isTDC && !isMSI && cat && <span style={{fontSize:10,color:C.sub}}>{cat.name}</span>}
          {acc && <Tag color={acc.color}>{acc.name}</Tag>}
          {pd && !isMSI && !isTDC && <Badge color={d!==null&&d<=3?C.red:C.blue}>💳 {fmtDateShort(pd)}</Badge>}
        </div>
      </div>
      <div style={{fontSize:14,fontWeight:800,color:amtColor,flexShrink:0}}>{isTDC?"-":""}{mxn(exp.amount)}</div>
    </div>
  );
}
...
function Expenses({expenses, setExpenses, accounts, setAccounts, subs, plans, setPlans, categories, goals, setGoals, session, reloadAll}) {
  ...
  const byMonth = useMemo(() => {
    ...
    expenses.forEach(exp => {
      ...
      if (!payGroups[key]) payGroups[key] = {mk, accountId:exp.accountId, paymentDate:exp.paymentDate, total:0, items:[]};
    ...
    Object.values(payGroups).forEach(pg => {
      ...
      const alreadyHas = map[pg.mk].some(e=>e.__payKey===pg.paymentDate+"|"+pg.accountId);
      if (!alreadyHas) {
        map[pg.mk].push({
          __isPaymentDue: true,
  ...
  const payDate = !form.isMSI && selectedAcc?.type==="credit"&&selectedAcc.cutDay&&selectedAcc.payDay
    ? calcPaymentDate(form.date, selectedAcc.cutDay, selectedAcc.payDay) : null;
  ...
  const save = async () => {
    ...
    if (form.isMSI && acc?.type==="credit") {
      ...
      const { data: planRow } = await sb.from("msi_plans").insert({
        user_id: userId, description: form.description.trim(),
        total_amount: amt, monthly_payment: monthly, total_months: numM,
        paid_months: 0, account_id: acc.id, start_date: form.date,
      }).select().single();
      ...
      if (planRow) {
        ...
        const rows = payDates.map((pd, i) => ({
          ...
          account_id: acc.id, category_id: form.categoryId || null,
          payment_date: pd, is_msi: true,
          msi_plan_id: planRow.id, msi_index: i+1, msi_total: numM,
    ...
    setForm(f=>({...f,description:"",amount:"",date:today(),isMSI:false,msiMonths:"9",linkedGoalId:null}));
    setShowModal(false);
    if (reloadAll) await reloadAll();
  };

  const totalThisMonth = expenses.filter(e=>e.date.startsWith(currentMonth)&&!e.isMSIInstallment).reduce((s,e)=>s+e.amount,0);
  ...
  return (
    <div style={{paddingBottom:80}}>
      ...
      <div style={{padding:"20px 20px 12px",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div>
          <div style={{fontSize:26,fontWeight:900,color:C.text}}>Gastos</div>
          <div style={{fontSize:13,color:C.sub,marginTop:2}}>
            Este mes: <span style={{color:C.red,fontWeight:800}}>{mxn(totalThisMonth)}</span>
          </div>
        </div>
        <div style={{display:"flex",gap:8}}>
          <button onClick={()=>setShowPayTDC(true)} style={{background:C.greenDim,border:`1px solid ${C.green}44`,borderRadius:12,padding:"0 12px",height:44,color:C.green,fontSize:12,fontWeight:800,cursor:"pointer",whiteSpace:"nowrap"}}>💳 Pagar TDC</button>
          <button onClick={()=>setShowModal(true)} style={{background:C.accent,border:"none",borderRadius:22,width:44,height:44,color:"#fff",fontSize:22,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
        </div>
      </div>
      ...
      <div style={{padding:"0 20px"}}>
        ...
        {visibleByMonth.map(([mk, allEntries])=>{
          ...
          // Payment-due entries: only show if today >= paymentDate
          const visiblePayments = paymentsDue.filter(p => daysUntil(p.paymentDate) <= 0);

          const headerCount = realExpenses.length + (visiblePayments.length>0 ? 1 : 0);

          return (
            <div key={mk} style={{marginBottom:10}}>
              {/* Month accordion header */}
              <button onClick={()=>toggleMonth(mk)} style={{
                width:"100%", background:isCurrent?C.accentDim:C.card,
                ...
                <div style={{flex:1,textAlign:"left"}}>
                  <div style={{fontSize:14,fontWeight:800,color:isCurrent?C.accent:C.text,textTransform:"capitalize"}}>
                    {isCurrent&&<span style={{fontSize:10,background:C.accent,color:"#fff",padding:"1px 6px",borderRadius:4,marginRight:6,fontWeight:700}}>ACTUAL</span>}
                    {monthLabel(mk)}
                  </div>
                  <div style={{display:"flex",gap:6,alignItems:"center",marginTop:2}}>
                    <span style={{fontSize:11,color:C.sub}}>{realExpenses.length} gasto{realExpenses.length!==1?"s":""}</span>
                    {visiblePayments.length>0&&(
                      <span style={{fontSize:9,background:C.red+"22",color:C.red,padding:"1px 5px",borderRadius:4,fontWeight:700}}>
                        💳 {visiblePayments.length} pago{visiblePayments.length!==1?"s":""} TDC
                      </span>
                    )}
                    {paymentsDue.filter(p=>daysUntil(p.paymentDate)>0).length>0&&(
                      <span style={{fontSize:9,background:C.blue+"22",color:C.blue,padding:"1px 5px",borderRadius:4,fontWeight:700}}>
                        🔒 {paymentsDue.filter(p=>daysUntil(p.paymentDate)>0).length} pendiente{paymentsDue.filter(p=>daysUntil(p.paymentDate)>0).length!==1?"s":""}
                      </span>
                    )}
                  </div>
                </div>
                <div style={{textAlign:"right",marginRight:4}}>
                  <div style={{fontSize:15,fontWeight:800,color:C.red}}>{mxn(monthTotal)}</div>
                </div>
                <div style={{fontSize:16,color:C.sub,transition:"transform .25s",transform:isOpen?"rotate(180deg)":"rotate(0deg)",flexShrink:0}}>▾</div>
              </button>
      ...
      <Modal open={showModal} onClose={()=>setShowModal(false)} title="Nuevo Gasto">
        <Field label="Descripción">
          <Input value={form.description} onChange={v=>setForm(f=>({...f,description:v}))} placeholder="Ej. iPhone, Starbucks..."/>
        </Field>
        <Field label="Monto Total (MXN)">
          <Input value={form.amount} onChange={v=>setForm(f=>({...f,amount:v}))} placeholder="0.00" type="number"/>
        </Field>
        <Field label="Fecha de compra">
          <Input value={form.date} onChange={v=>setForm(f=>({...f,date:v}))} type="date"/>
        </Field>
        <Field label="Cuenta">
          <ChipSelect options={accounts} value={form.accountId} onChange={v=>setForm(f=>({...f,accountId:v,isMSI:false}))} getColor={a=>a.color}/>
        </Field>
        <Field label="Categoría">
          <ChipSelect options={categories} value={form.categoryId} onChange={v=>setForm(f=>({...f,categoryId:v,linkedGoalId:null,isMSI:false}))} getColor={c=>c.color}/>
        </Field>
          ...
          <Field label="¿A qué meta va este ahorro?" hint="Requerido">
            <div style={{display:"flex",flexDirection:"column",gap:7}}>
              {goals.map(g=>{
                ...
                return (
                  <button key={g.id} onClick={()=>setForm(f=>({...f,linkedGoalId:g.id}))} style={{
                    display:"flex",alignItems:"center",gap:10,
                    ...
                    <div style={{flex:1,textAlign:"left"}}>
                      <div style={{fontSize:13,fontWeight:800,color:C.text}}>{g.name}</div>
                      <div style={{fontSize:10,color:C.sub}}>{mxn(g.current)} / {mxn(g.target)} · {pct.toFixed(0)}%</div>
                    </div>
                    {sel&&<span style={{fontSize:14,color:g.color}}>✓</span>}
                  </button>
                );
              })}
              {goals.length===0&&<div style={{fontSize:12,color:C.muted,textAlign:"center",padding:"8px 0"}}>No tienes metas creadas</div>}
            </div>
            {form.linkedGoalId&&form.amount&&!isNaN(parseFloat(form.amount))&&(()=>{
              const g=goals.find(x=>x.id===form.linkedGoalId);
              const amt=parseFloat(form.amount);
              const nomina=accounts.find(a=>a.type==="debit");
              if(!g) return null;
              return (
                <div style={{background:C.greenDim,borderRadius:10,padding:"9px 12px",marginTop:8,border:`1px solid ${C.green}44`}}>
                  <div style={{fontSize:10,color:C.green,fontWeight:700,marginBottom:4}}>✓ EFECTO DEL AHORRO</div>
                  <div style={{fontSize:12,color:C.sub}}>{g.icon} {g.name}: <span style={{color:C.green,fontWeight:800}}>{mxn(g.current)} → {mxn(g.current+amt)}</span></div>
                  {nomina&&<div style={{fontSize:12,color:C.sub,marginTop:3}}>🏦 {nomina.name}: <span style={{color:C.red,fontWeight:800}}>{mxn(nomina.balance)} → {mxn(nomina.balance-amt)}</span></div>}
                </div>
              );
            })()}
          </Field>
        ...
        {selectedAcc?.type==="credit"&&!isSavingsCat&&(
          <div style={{marginBottom:14}}>
            <button onClick={()=>setForm(f=>({...f,isMSI:!f.isMSI}))} style={{
              width:"100%", background:form.isMSI?C.orange+"22":C.card,
              ...
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                <div style={{textAlign:"left"}}>
                  <div style={{fontSize:13,fontWeight:800,color:form.isMSI?C.orange:C.text}}>Compra a meses sin intereses</div>
                  <div style={{fontSize:10,color:C.sub}}>Divide el pago en mensualidades</div>
                </div>
              </div>
              <div style={{width:28,height:16,borderRadius:8,background:form.isMSI?C.orange:C.border,position:"relative",transition:"background .2s",flexShrink:0}}>
                <div style={{position:"absolute",top:2,left:form.isMSI?12:2,width:12,height:12,borderRadius:6,background:"#fff",transition:"left .2s"}}/>
              </div>
            </button>
          </div>
        )}
        ...
        {form.isMSI&&selectedAcc?.type==="credit"&&(
          <>
            ...
            {msiTotal>0&&msiMonthsNum>0&&selectedAcc.cutDay&&selectedAcc.payDay&&(()=>{
              const dates = getMsiPaymentDates(form.date, selectedAcc, msiMonthsNum);
              return (
                <div style={{background:C.orange+"11",borderRadius:14,padding:"12px 14px",marginBottom:14,border:`1.5px solid ${C.orange}44`}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                    <span style={{fontSize:11,fontWeight:800,color:C.orange,textTransform:"uppercase",letterSpacing:.5}}>📆 Plan MSI</span>
                    <span style={{fontSize:12,fontWeight:800,color:C.text}}>{mxn(msiTotal)} ÷ {msiMonthsNum}</span>
                  </div>
                  <div style={{display:"flex",justifyContent:"space-between",background:C.elevated,borderRadius:10,padding:"10px 14px",marginBottom:10}}>
                    <div style={{textAlign:"center"}}>
                      <div style={{fontSize:20,fontWeight:900,color:C.orange}}>{mxn(msiMonthly)}</div>
                      <div style={{fontSize:10,color:C.sub}}>por mes</div>
                    </div>
                    <div style={{textAlign:"center"}}>
                      <div style={{fontSize:20,fontWeight:900,color:C.text}}>{msiMonthsNum}</div>
                      <div style={{fontSize:10,color:C.sub}}>meses</div>
                    </div>
                    <div style={{textAlign:"center"}}>
                      <div style={{fontSize:20,fontWeight:900,color:C.text}}>{mxn(msiTotal)}</div>
                      <div style={{fontSize:10,color:C.sub}}>total</div>
                    </div>
                  </div>
                  <div style={{fontSize:11,fontWeight:700,color:C.sub,marginBottom:6}}>Calendario de pagos:</div>
                  <div style={{display:"flex",flexDirection:"column",gap:4,maxHeight:140,overflowY:"auto"}}>
                    {dates.map((d,i)=>(
                      <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:C.card,borderRadius:8,padding:"5px 10px"}}>
                        <span style={{fontSize:11,color:C.sub}}>Pago {i+1}/{msiMonthsNum}</span>
                        <span style={{fontSize:11,fontWeight:700,color:C.text}}>{fmtDate(d)}</span>
                        <span style={{fontSize:11,fontWeight:800,color:C.orange}}>{mxn(msiMonthly)}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {!form.isMSI&&payDate&&(
          <div style={{background:C.accentDim,borderRadius:12,padding:"10px 14px",marginBottom:14,border:`1px solid ${C.accent}44`}}>
            <div style={{fontSize:11,color:C.accent,fontWeight:700,marginBottom:3}}>📅 FECHA DE PAGO ESTIMADA</div>
            <div style={{fontSize:16,fontWeight:900,color:C.text}}>{fmtDate(payDate)}</div>
            <div style={{fontSize:11,color:C.sub,marginTop:2}}>
              Ciclo que corta el día <b>{selectedAcc?.cutDay}</b> · vence día <b>{selectedAcc?.payDay}</b>
            </div>
          </div>
        )}

        <SaveBtn onClick={save} color={form.isMSI?C.orange:C.accent}>
          {form.isMSI ? `Registrar MSI · ${msiMonthsNum} pagos de ${mxn(msiMonthly)}` : "Registrar Gasto"}
        </SaveBtn>
      </Modal>
      ...
      {/* PAY TDC MODAL */}
      <Modal open={showPayTDC} onClose={()=>setShowPayTDC(false)} title="💳 Pagar Tarjeta de Crédito">
        <div style={{fontSize:12,color:C.sub,marginBottom:16}}>
          El pago se descuenta de tu cuenta <b style={{color:"#4CAF50"}}>Nómina</b> y reduce la deuda de la tarjeta.
        </div>
        <Field label="Tarjeta a pagar">
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {accounts.filter(a=>a.type==="credit").map(acc=>{
              ...
              return (
                <button key={acc.id} onClick={()=>setPayTDCForm(f=>({...f,accountId:acc.id}))} style={{
                  display:"flex",alignItems:"center",gap:10,
                  ...
                  <div style={{flex:1,textAlign:"left"}}>
                    <div style={{fontSize:13,fontWeight:800,color:C.text}}>{acc.name}</div>
                    <div style={{fontSize:11,color:C.red}}>Deuda: {mxn(Math.abs(acc.balance))}</div>
                  </div>
                  {selected&&<span style={{fontSize:16,color:acc.color}}>✓</span>}
                </button>
              );
            })}
          </div>
        ...
        <Field label="Monto del pago (MXN)">
          <Input value={payTDCForm.amount} onChange={v=>setPayTDCForm(f=>({...f,amount:v}))} placeholder="0.00" type="number"/>
        </Field>
        {payTDCForm.amount&&!isNaN(parseFloat(payTDCForm.amount))&&(()=>{
          ...
          return (
            <div style={{background:insufficient?C.redDim:C.elevated,borderRadius:12,padding:"12px 14px",marginBottom:14,border:`1px solid ${insufficient?C.red+"66":C.border}`}}>
              {insufficient
                ? <div style={{fontSize:12,color:C.red,fontWeight:800,marginBottom:4}}>⚠️ SALDO INSUFICIENTE EN {nomina?.name?.toUpperCase()} — Faltan {mxn(amt-nomina.balance)}</div>
                : <div style={{fontSize:11,color:C.green,fontWeight:700,marginBottom:10}}>✓ IMPACTO DEL PAGO</div>
              }
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  <span style={{fontSize:14}}>💳</span>
                  <span style={{fontSize:12,color:C.sub}}>{tdc?.name}</span>
                </div>
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  <span style={{fontSize:12,fontWeight:700,color:C.red}}>{mxn(Math.abs(tdc?.balance||0))}</span>
                  <span style={{fontSize:11,color:C.muted}}>→</span>
                  <span style={{fontSize:12,fontWeight:800,color:newTDC===0?C.green:C.orange}}>{mxn(Math.abs(newTDC))}</span>
                </div>
              </div>
              {nomina&&<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",paddingTop:8,borderTop:`1px solid ${C.border}`}}>
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  <span style={{fontSize:14}}>🏦</span>
                  <span style={{fontSize:12,color:C.sub}}>{nomina.name}</span>
                </div>
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  <span style={{fontSize:12,fontWeight:700,color:C.text}}>{mxn(nomina.balance)}</span>
                  <span style={{fontSize:11,color:C.muted}}>→</span>
                  <span style={{fontSize:12,fontWeight:800,color:newNomina>=0?C.text:C.red}}>{mxn(newNomina)}</span>
                </div>
              </div>}
            </div>
          );
        ...
        <SaveBtn onClick={async()=>{
          ...
          // Register expense
          await sb.from("expenses").insert({ user_id: session?.user?.id, description: `Pago ${tdcAcc?.name||"TDC"}`, amount: amt, date: today(), account_id: nominaAcc?.id||tdcAcc.id, category_id: null, payment_date: null, is_msi: false, is_tdc_payment: true, is_subscription: false });
          setPayTDCForm(f=>({...f,amount:""})); setShowPayTDC(false);
          if(reloadAll) await reloadAll();
        }} color={C.green}>Registrar Pago</SaveBtn>
      </Modal>
    </div>
  );
}
...
// ─── SUBSCRIPTIONS ────────────────────────────────────────────────────────────
function Subscriptions({subs, setSubs, accounts, expenses, setExpenses, categories, session, reloadAll}) {
  const [showModal, setShowModal] = useState(false);
  const [editingSub, setEditingSub] = useState(null);
  const [nextId, setNextId] = useState(50);
  const [nextExpId, setNextExpId] = useState(300);
  const emptyForm = {name:"",amount:"",frequency:"monthly",categoryId:1,accountId:accounts[0]?.id||2,chargeDay:1};
  const [form, setForm] = useState(emptyForm);

  const totalMonthly = subs.filter(s=>s.active&&s.frequency==="monthly").reduce((s,sub)=>s+sub.amount,0);
  const currentMonth = getCurrentMonth();

  const chargeThisMonth = (sub) => {
    const chargeDate = `${currentMonth}-${String(sub.chargeDay).padStart(2,'0')}`;
    return !expenses.some(e=>e.description===sub.name&&e.date===chargeDate&&e.accountId===sub.accountId);
  };
  ...
  const openAdd = () => { setEditingSub(null); setForm(emptyForm); setShowModal(true); };
  const openEdit = (sub) => {
    setEditingSub(sub);
    setForm({name:sub.name,amount:String(sub.amount),frequency:sub.frequency,categoryId:sub.categoryId,accountId:sub.accountId,chargeDay:sub.chargeDay});
    setShowModal(true);
  };

  const save = async () => {
    if(!form.name.trim()||!form.amount) return;
    const data = { name: form.name.trim(), amount: parseFloat(form.amount), frequency: form.frequency, category_id: form.categoryId||null, account_id: form.accountId||null, charge_day: form.chargeDay, active: true };
    if (editingSub) {
      await sb.from("subscriptions").update(data).eq("id", editingSub.id);
    } else {
      await sb.from("subscriptions").insert({ ...data, user_id: session?.user?.id });
    }
    setShowModal(false);
    if (reloadAll) await reloadAll();
  };
  ...
  const SubRow = ({sub}) => {
    ...
    const pd = acc?.type==="credit"&&acc.cutDay&&acc.payDay ? calcPaymentDate(next,acc.cutDay,acc.payDay) : null;
    return (
      <div style={{background:C.card,borderRadius:14,padding:"12px 14px",marginBottom:8,border:`1px solid ${d<=3?C.orange+"44":C.border}`}}>
        <div style={{display:"flex",alignItems:"center",gap:11}}>
          <div style={{width:40,height:40,borderRadius:11,background:C.accentDim,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>🔄</div>
          <div style={{flex:1}}>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}>
              <span style={{fontSize:14,fontWeight:800,color:C.text}}>{sub.name}</span>
              {!notCharged&&<Badge color={C.green}>✓ Cobrado</Badge>}
              {d<=3&&notCharged&&<Badge color={C.orange}>Próximo {d===0?"hoy":`${d}d`}</Badge>}
            </div>
            <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
              {acc&&<Tag color={acc.color}>{acc.name}</Tag>}
              {cat&&<span style={{fontSize:10,color:C.sub}}>{cat.name}</span>}
              <span style={{fontSize:10,color:C.muted}}>Día {sub.chargeDay} c/mes</span>
            </div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:15,fontWeight:800,color:C.text}}>{mxn(sub.amount)}</div>
            {pd&&<div style={{fontSize:9,color:C.blue,marginTop:2}}>Paga: {fmtDateShort(pd)}</div>}
          </div>
          <button onClick={()=>openEdit(sub)} style={{background:C.elevated,border:`1px solid ${C.border}`,borderRadius:8,padding:"6px 8px",color:C.sub,fontSize:13,cursor:"pointer",flexShrink:0}}>✏️</button>
        </div>
        {notCharged&&(
          <button onClick={()=>manualCharge(sub)} style={{width:"100%",marginTop:10,background:C.accentDim,border:`1px solid ${C.accent}44`,borderRadius:10,padding:"8px 0",color:C.accent,fontSize:12,fontWeight:700,cursor:"pointer"}}>
  ...
  const previewPayDate = () => {
    ...
    if(acc?.type==="credit"&&acc.cutDay&&acc.payDay){
      const pd = calcPaymentDate(nextOccurrence(form.chargeDay),acc.cutDay,acc.payDay);
      return <div style={{background:C.accentDim,borderRadius:10,padding:"9px 12px",marginBottom:14,border:`1px solid ${C.accent}33`}}>
        <div style={{fontSize:10,color:C.accent,fontWeight:700}}>📅 Aparece en estado de cuenta a pagar el <b>{fmtDate(pd)}</b></div>
      </div>;
    }
    return null;
  };
  ...
  return (
    <div style={{paddingBottom:80}}>
      <div style={{padding:"20px 20px 10px",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div>
          <div style={{fontSize:26,fontWeight:900,color:C.text}}>Suscripciones</div>
          <div style={{fontSize:13,color:C.sub,marginTop:2}}>{mxn(totalMonthly)}/mes · {mxn(totalMonthly*12)}/año</div>
        </div>
        <button onClick={openAdd} style={{background:C.accent,border:"none",borderRadius:22,width:44,height:44,color:"#fff",fontSize:22,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
      </div>
      <div style={{padding:"0 20px 16px",display:"flex",gap:10}}>
        {[
          {icon:"🔄",val:subs.filter(s=>s.frequency==="monthly").length,label:"Mensuales",color:C.accent},
          {icon:"💸",val:mxn(totalMonthly,true),label:"Gasto/mes",color:C.red},
          {icon:"📅",val:mxn(totalMonthly*12,true),label:"Al año",color:C.orange},
        ].map((s,i)=>(
          <div key={i} style={{flex:1,background:C.card,borderRadius:14,padding:"10px 8px",border:`1px solid ${C.border}`,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
      ...
      <div style={{padding:"0 20px"}}>
        {subs.map(sub=><SubRow key={sub.id} sub={sub}/>)}
        {subs.length===0&&<div style={{textAlign:"center",paddingTop:60,color:C.muted}}><div style={{fontSize:40}}>🔄</div><div style={{marginTop:10}}>Sin suscripciones</div></div>}
      </div>

      <Modal open={showModal} onClose={()=>setShowModal(false)} title={editingSub?"Editar Suscripción":"Nueva Suscripción"}>
        <Field label="Nombre del servicio">
          <Input value={form.name} onChange={v=>setForm(f=>({...f,name:v}))} placeholder="Ej. Netflix, Gym..."/>
        </Field>
        <Field label="Monto mensual (MXN)">
          <Input value={form.amount} onChange={v=>setForm(f=>({...f,amount:v}))} placeholder="0.00" type="number"/>
        </Field>
        <Field label="Día de cobro" hint="¿Qué día del mes te cobran?">
          <Stepper value={form.chargeDay} onChange={v=>setForm(f=>({...f,chargeDay:v}))} min={1} max={31}/>
          <p style={{fontSize:11,color:C.sub,margin:"6px 0 0"}}>Próximo cargo: <b style={{color:C.orange}}>{fmtDate(nextOccurrence(form.chargeDay))}</b></p>
        </Field>
        <Field label="Cuenta de cargo">
          <ChipSelect options={accounts} value={form.accountId} onChange={v=>setForm(f=>({...f,accountId:v}))} getColor={a=>a.color}/>
        </Field>
        {previewPayDate()}
        <Field label="Categoría">
          <ChipSelect options={categories} value={form.categoryId} onChange={v=>setForm(f=>({...f,categoryId:v}))} getColor={c=>c.color}/>
        </Field>
        {editingSub&&(
          <button onClick={deleteSub} style={{width:"100%",background:C.redDim,border:`1px solid ${C.red}44`,borderRadius:12,padding:"11px 0",color:C.red,fontSize:13,fontWeight:700,cursor:"pointer",marginBottom:8}}>
            🗑 Eliminar suscripción
          </button>
        )}
        <SaveBtn onClick={save}>{editingSub?"Guardar Cambios":"Guardar Suscripción"}</SaveBtn>
      </Modal>
    </div>
  );
}
...
function Goals({goals, setGoals, accounts, setAccounts, transfers, session, reloadAll}) {
  ...
  const openEdit = (goal) => {
    setEditing(goal);
    setEditForm({name:goal.name,target:String(goal.target),current:String(goal.current),icon:goal.icon,color:goal.color});
  };

  const saveEdit = async () => {
    const cur = parseFloat(editForm.current); const tar = parseFloat(editForm.target);
    if(!editForm.name||isNaN(tar)||tar<=0) return;
    await sb.from("goals").update({ name:editForm.name, target_amount:tar, current_amount:isNaN(cur)?editing.current:cur, icon:editForm.icon, color:editForm.color }).eq("id", editing.id);
    setEditing(null);
    if (reloadAll) await reloadAll();
  };
  ...
  const saveAdd = async () => {
    ...
    await sb.from("goals").insert({ user_id: session?.user?.id, name:addForm.name, target_amount:parseFloat(addForm.target), current_amount:parseFloat(addForm.current)||0, icon:addForm.icon, color:addForm.color });
    setAddForm({name:"",target:"",current:"0",icon:"🎯",color:"#00D08A"});
    setShowAdd(false);
    if (reloadAll) await reloadAll();
  };
  ...
  const ColorPicker = ({value, onChange}) => (
    <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
      {COLS.map(col=><button key={col} onClick={()=>onChange(col)} style={{width:34,height:34,borderRadius:17,background:col,border:"none",cursor:"pointer",outline:value===col?"3px solid #fff":"none",outlineOffset:2}}/>)}
    </div>
  );
  const IconPicker = ({value, onChange, color}) => (
    <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
      {ICONS.map(ic=><button key={ic} onClick={()=>onChange(ic)} style={{width:40,height:40,borderRadius:10,border:"none",background:value===ic?color+"44":C.card,fontSize:20,cursor:"pointer",outline:value===ic?`2px solid ${color}`:`1px solid ${C.border}`}}>{ic}</button>)}
    </div>
  );
  ...
  return (
    <div style={{paddingBottom:80}}>
      <div style={{padding:"20px 20px 10px",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div>
          <div style={{fontSize:26,fontWeight:900,color:C.text}}>Metas</div>
          <div style={{fontSize:13,color:C.sub,marginTop:2}}>{mxn(goals.reduce((s,g)=>s+g.current,0))} ahorrado</div>
        </div>
        <div style={{display:"flex",gap:8}}>
          <button onClick={()=>setShowWithdraw(true)} style={{background:C.orangeDim,border:`1px solid ${C.orange}44`,borderRadius:22,width:44,height:44,color:C.orange,fontSize:20,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>💸</button>
          <button onClick={()=>setShowAdd(true)} style={{background:C.green,border:"none",borderRadius:22,width:44,height:44,color:"#fff",fontSize:22,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
        </div>
      </div>

      <div style={{padding:"0 20px",display:"flex",flexDirection:"column",gap:10}}>
        {goals.map(goal=>{
          const pct=goal.target>0?(goal.current/goal.target)*100:0; const done=pct>=100;
          return (
            <div key={goal.id} style={{background:C.card,borderRadius:18,padding:16,border:`1px solid ${done?goal.color+"55":C.border}`}}>
              <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
                <div style={{width:46,height:46,borderRadius:13,background:goal.color+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0}}>{goal.icon}</div>
                <div style={{flex:1}}>
                  <div style={{fontSize:15,fontWeight:800,color:C.text}}>{goal.name}</div>
                  {done&&<span style={{fontSize:10,color:C.green,fontWeight:800}}>✓ META ALCANZADA</span>}
                </div>
                <div style={{fontSize:22,fontWeight:900,color:done?C.green:goal.color}}>{pct.toFixed(0)}%</div>
              </div>
              <ProgressBar pct={pct} color={done?C.green:goal.color} h={8}/>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:10}}>
                <div>
                  <span style={{fontSize:13,fontWeight:700,color:C.text}}>{mxn(goal.current)}</span>
                  <span style={{fontSize:11,color:C.sub}}> / {mxn(goal.target)}</span>
                </div>
                <div style={{display:"flex",gap:6}}>
                  <button onClick={()=>openWithdraw(goal)} style={{background:C.orangeDim,border:`1px solid ${C.orange}44`,borderRadius:8,padding:"5px 9px",color:C.orange,fontSize:12,fontWeight:700,cursor:"pointer"}}>💸 Retirar</button>
        ...
        {goals.length===0&&<div style={{textAlign:"center",paddingTop:60,color:C.muted}}><div style={{fontSize:40}}>🎯</div><div style={{marginTop:10}}>Sin metas</div></div>}
      </div>

      {/* EDIT MODAL */}
      <Modal open={!!editing} onClose={()=>setEditing(null)} title="Editar Meta">
        <Field label="Nombre"><Input value={editForm.name} onChange={v=>setEditForm(f=>({...f,name:v}))} placeholder="Nombre..."/></Field>
        <Field label="Monto Objetivo (MXN)"><Input value={editForm.target} onChange={v=>setEditForm(f=>({...f,target:v}))} placeholder="50000" type="number"/></Field>
        <Field label="Monto Actual (MXN)"><Input value={editForm.current} onChange={v=>setEditForm(f=>({...f,current:v}))} placeholder="0" type="number"/></Field>
        <Field label="Ícono"><IconPicker value={editForm.icon} onChange={v=>setEditForm(f=>({...f,icon:v}))} color={editForm.color}/></Field>
        <Field label="Color"><ColorPicker value={editForm.color} onChange={v=>setEditForm(f=>({...f,color:v}))}/></Field>
        <button onClick={deleteGoal} style={{width:"100%",background:C.redDim,border:`1px solid ${C.red}44`,borderRadius:12,padding:"11px 0",color:C.red,fontSize:13,fontWeight:700,cursor:"pointer",marginBottom:8,marginTop:4}}>🗑 Eliminar meta</button>
        <SaveBtn onClick={saveEdit} color={editForm.color}>Guardar Cambios</SaveBtn>
      </Modal>

      {/* WITHDRAW MODAL */}
      <Modal open={showWithdraw&&!editForm.name} onClose={()=>{setShowWithdraw(false);setWithdrawGoal(null);}} title="Retirar de Meta">
        {!withdrawGoal&&goals.length>0&&(
          <>
            <div style={{fontSize:12,color:C.sub,marginBottom:12}}>¿De qué meta retiras?</div>
            {goals.map(g=>(
              <button key={g.id} onClick={()=>setWithdrawGoal(g)} style={{width:"100%",display:"flex",alignItems:"center",gap:10,background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 14px",marginBottom:8,cursor:"pointer"}}>
                <span style={{fontSize:22}}>{g.icon}</span>
                <div style={{flex:1,textAlign:"left"}}>
                  <div style={{fontSize:13,fontWeight:800,color:C.text}}>{g.name}</div>
                  <div style={{fontSize:11,color:C.sub}}>{mxn(g.current)} disponible</div>
                </div>
              </button>
            ))}
          </>
        ...
        {withdrawGoal&&(
          <>
            <div style={{background:withdrawGoal.color+"22",borderRadius:12,padding:"10px 14px",marginBottom:16,display:"flex",alignItems:"center",gap:10}}>
              <span style={{fontSize:22}}>{withdrawGoal.icon}</span>
              <div>
                <div style={{fontSize:13,fontWeight:800,color:C.text}}>{withdrawGoal.name}</div>
                <div style={{fontSize:11,color:C.sub}}>Disponible: {mxn(withdrawGoal.current)}</div>
              </div>
            </div>
            ...
            <Field label="Enviar a cuenta de débito">
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {debitAccounts.map(acc=>(
                  <button key={acc.id} onClick={()=>doWithdraw(acc.id)} style={{
                    display:"flex",alignItems:"center",gap:10,
                    background:C.card,border:`1.5px solid ${acc.color}44`,
                    borderRadius:12,padding:"12px 14px",cursor:"pointer",
                  }}>
                    <span style={{fontSize:20}}>🏦</span>
                    <div style={{flex:1,textAlign:"left"}}>
                      <div style={{fontSize:13,fontWeight:800,color:C.text}}>{acc.name}</div>
                      <div style={{fontSize:11,color:C.sub}}>Saldo: {mxn(acc.balance)}</div>
                    </div>
                    <div style={{fontSize:12,fontWeight:800,color:C.green}}>
                      {withdrawAmt&&!isNaN(parseFloat(withdrawAmt))&&`→ ${mxn(acc.balance+parseFloat(withdrawAmt))}`}
                    </div>
                  </button>
                ))}
                {debitAccounts.length===0&&<div style={{color:C.muted,fontSize:12,textAlign:"center",padding:"10px 0"}}>No tienes cuentas de débito</div>}
              </div>
            </Field>
            <button onClick={()=>setWithdrawGoal(null)} style={{width:"100%",background:"none",border:"none",color:C.sub,fontSize:12,cursor:"pointer",marginTop:4}}>← Cambiar meta</button>
          </>
        )}
      </Modal>

      {/* ADD MODAL */}
      <Modal open={showAdd} onClose={()=>setShowAdd(false)} title="Nueva Meta">
        <Field label="Nombre"><Input value={addForm.name} onChange={v=>setAddForm(f=>({...f,name:v}))} placeholder="Ej. Viaje a Tokio..."/></Field>
        <Field label="Monto Objetivo"><Input value={addForm.target} onChange={v=>setAddForm(f=>({...f,target:v}))} placeholder="50000" type="number"/></Field>
        <Field label="Monto Actual"><Input value={addForm.current} onChange={v=>setAddForm(f=>({...f,current:v}))} placeholder="0" type="number"/></Field>
        <Field label="Ícono"><IconPicker value={addForm.icon} onChange={v=>setAddForm(f=>({...f,icon:v}))} color={addForm.color}/></Field>
        <Field label="Color"><ColorPicker value={addForm.color} onChange={v=>setAddForm(f=>({...f,color:v}))}/></Field>
        <SaveBtn onClick={saveAdd} color={addForm.color}>Crear Meta</SaveBtn>
      </Modal>
    </div>
  );
}
...
function MSI({plans, setPlans, accounts, session, reloadAll}) {
  ...
  const SectionHeader = ({label, count, total, isOpen, onToggle, color=C.accent}) => (
    <button onClick={onToggle} style={{
      ...
      background:C.card,border:`1px solid ${isOpen?color+"55":C.border}`,
      borderRadius: isOpen?"14px 14px 0 0":14,
      padding:"12px 16px",cursor:"pointer",marginBottom:isOpen?0:10,
      transition:"border-radius .2s",
    }}>
      <div style={{flex:1,textAlign:"left"}}>
        <div style={{fontSize:14,fontWeight:800,color:isOpen?color:C.text}}>{label}</div>
        <div style={{fontSize:11,color:C.sub,marginTop:2}}>{count} plan{count!==1?"es":""}</div>
      </div>
      {total>0&&<div style={{fontSize:14,fontWeight:800,color}}>{mxn(total,true)}</div>}
      <div style={{fontSize:15,color:C.sub,transition:"transform .25s",transform:isOpen?"rotate(180deg)":"rotate(0deg)"}}>▾</div>
    </button>
  );
...
function Categories({categories, setCategories}) {
  ...
  const openEdit = (cat) => {
    setEditing(cat);
    setForm({name:cat.name, budget:String(cat.budget), icon:cat.icon, color:cat.color});
    setShowModal(true);
  };
  ...
  const save = async () => {
    ...
    if(editing) {
      await sb.from("categories").update({ name:form.name.trim(), budget, icon:form.icon, color:form.color }).eq("id", editing.id);
    } else {
      await sb.from("categories").insert({ user_id: session?.user?.id, name:form.name.trim(), budget, icon:form.icon, color:form.color });
    }
    setShowModal(false);
    if (reloadAll) await reloadAll();
  };
  ...
  return (
    <div style={{paddingBottom:80}}>
      <div style={{padding:"20px 20px 12px",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div>
          <div style={{fontSize:26,fontWeight:900,color:C.text}}>Categorías</div>
          <div style={{fontSize:13,color:C.sub,marginTop:2}}>
            Presupuesto total: <span style={{color:C.accent,fontWeight:800}}>{mxn(categories.reduce((s,c)=>s+c.budget,0))}</span>
          </div>
        </div>
        <button onClick={openAdd} style={{background:C.accent,border:"none",borderRadius:22,width:44,height:44,color:"#fff",fontSize:22,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
      </div>
      ...
      <div style={{padding:"0 20px",display:"flex",flexDirection:"column",gap:8}}>
        {categories.map(cat=>(
          <div key={cat.id} style={{background:C.card,borderRadius:16,padding:"13px 14px",border:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:12}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:12,color:C.sub,marginTop:2}}>
                Presupuesto: <span style={{color:cat.color,fontWeight:700}}>{mxn(cat.budget)}</span>/mes
              </div>
            </div>
            <button onClick={()=>openEdit(cat)} style={{background:C.elevated,border:`1px solid ${C.border}`,borderRadius:10,padding:"7px 11px",color:C.sub,fontSize:13,cursor:"pointer",flexShrink:0}}>✏️</button>
          </div>
        ))}
        {categories.length===0&&(
      ...
      <Modal open={showModal} onClose={()=>setShowModal(false)} title={editing?"Editar Categoría":"Nueva Categoría"}>
        <Field label="Nombre">
          <Input value={form.name} onChange={v=>setForm(f=>({...f,name:v}))} placeholder="Ej. Transporte, Salud..."/>
        </Field>
        <Field label="Presupuesto mensual (MXN)" hint="Cuánto puedes gastar aquí">
          <Input value={form.budget} onChange={v=>setForm(f=>({...f,budget:v}))} placeholder="0.00" type="number"/>
        </Field>
        <Field label="Ícono">
          <div style={{display:"flex",flexWrap:"wrap",gap:7}}>
            {CAT_ICONS.map(ic=>(
              <button key={ic} onClick={()=>setForm(f=>({...f,icon:ic}))} style={{
                width:40,height:40,borderRadius:10,border:"none",fontSize:20,cursor:"pointer",
                background:form.icon===ic?form.color+"44":C.card,
                outline:form.icon===ic?`2px solid ${form.color}`:`1px solid ${C.border}`,
              }}>{ic}</button>
            ))}
          </div>
        </Field>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:2}}>
            {CAT_COLORS.map(col=>(
              <button key={col} onClick={()=>setForm(f=>({...f,color:col}))} style={{
                width:34,height:34,borderRadius:17,background:col,border:"none",cursor:"pointer",
                outline:form.color===col?"3px solid #fff":"2px solid transparent",outlineOffset:2,transition:"all .15s",
              }}/>
            ))}
          </div>
        </Field>
        ...
        {/* Live preview */}
        <div style={{background:C.card,borderRadius:12,padding:"11px 14px",border:`1px solid ${C.border}`,borderLeft:`3px solid ${form.color}`,marginBottom:12,display:"flex",alignItems:"center",gap:10}}>
          <span style={{fontSize:22}}>{form.icon}</span>
          <div>
            <div style={{fontSize:13,fontWeight:800,color:C.text}}>{form.name||"Nueva categoría"}</div>
            <div style={{fontSize:11,color:form.color,fontWeight:700}}>{form.budget?mxn(parseFloat(form.budget)||0):mxn(0)}/mes</div>
          </div>
        </div>

        {editing&&(
          <button onClick={deleteCat} style={{width:"100%",background:C.redDim,border:`1px solid ${C.red}44`,borderRadius:12,padding:"11px 0",color:C.red,fontSize:13,fontWeight:700,cursor:"pointer",marginBottom:8}}>
            🗑 Eliminar categoría
          </button>
        )}
        <SaveBtn onClick={save} color={form.color}>{editing?"Guardar Cambios":"Crear Categoría"}</SaveBtn>
      </Modal>
    </div>
  );
}
...
function AuthScreen({ onLogin }) {
  ...
  const inputStyle = {background:C.elevated,border:`1px solid ${C.border}`,borderRadius:12,padding:"13px 14px",color:C.text,fontSize:14,width:"100%",outline:"none",marginBottom:14};
  const labelStyle = {fontSize:11,fontWeight:700,color:C.sub,textTransform:"uppercase",letterSpacing:0.8,marginBottom:6,display:"block"};
  ...
  const handleRegister = async () => {
    ...
    else setMsg("✅ Cuenta creada. Revisa tu email para confirmar, luego inicia sesión.");
    setLoading(false);
  };

  return (
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",padding:24,fontFamily:"-apple-system,'SF Pro Display','Segoe UI',sans-serif"}}>
      <style>{`*{box-sizing:border-box;}body{margin:0;background:${C.bg};}input::placeholder{color:${C.muted};}`}</style>
      <div style={{width:"100%",maxWidth:400}}>
        {/* Logo */}
        <div style={{textAlign:"center",marginBottom:36}}>
          <div style={{fontSize:56,marginBottom:12}}>💰</div>
          <div style={{fontSize:26,fontWeight:900,color:C.text,letterSpacing:-0.5}}>Finance Tracker</div>
          <div style={{fontSize:14,color:C.sub,marginTop:4}}>Tus finanzas, bajo control</div>
        </div>
        ...
        <div style={{background:C.card,borderRadius:20,padding:28,border:`1px solid ${C.border}`}}>
          ...
          {mode==="register" && <>
            ...
            <input style={inputStyle} placeholder="Tu nombre" value={name} onChange={e=>setName(e.target.value)} autoComplete="name"/>
          </>}

          <label style={labelStyle}>Email</label>
          <input style={inputStyle} placeholder="tu@email.com" value={email} onChange={e=>setEmail(e.target.value)} type="email" autoComplete="email"/>
          ...
          {mode==="register" && <>
            ...
            <input style={inputStyle} placeholder="Repite la contraseña" value={confirm} onChange={e=>setConfirm(e.target.value)} type="password"/>
          </>}
          ...
          <button
            ...
            disabled={loading}
            style={{width:"100%",background:C.accent,border:"none",borderRadius:12,padding:15,color:"#fff",fontSize:15,fontWeight:800,cursor:"pointer",opacity:loading?0.6:1}}
          >
          ...
          <div style={{textAlign:"center",marginTop:16}}>
            <button onClick={()=>{setMode(mode==="login"?"register":"login");setError("");setMsg("");}}
              style={{background:"none",border:"none",cursor:"pointer",fontSize:13,color:C.sub}}>
              {mode==="login" ? <span>¿No tienes cuenta? <span style={{color:C.accent,fontWeight:700}}>Crear una</span></span>
                              : <span>¿Ya tienes cuenta? <span style={{color:C.accent,fontWeight:700}}>Iniciar sesión</span></span>}
            </button>
          </div>
        </div>
      </div>
...
export default function App() {
  ...
  if (isDesktop) return (
    <div style={{display:"flex",minHeight:"100vh",background:C.bg,fontFamily:"-apple-system,'SF Pro Display','Segoe UI',sans-serif"}}>
      <style>{globalStyles+`::-webkit-scrollbar{width:6px;}::-webkit-scrollbar-track{background:${C.bg};}::-webkit-scrollbar-thumb{background:${C.border};border-radius:3px;}`}</style>
      <Sidebar tab={tab} setTab={setTab} userName={userName} onLogout={handleLogout}/>
  ...
  return (
    <div style={{background:C.bg,minHeight:"100vh",maxWidth:480,margin:"0 auto",position:"relative",fontFamily:"-apple-system,'SF Pro Display','Segoe UI',sans-serif"}}>
      ...
      <div style={{display:"flex",justifyContent:"flex-end",padding:"6px 16px 0",background:C.bg}}>
